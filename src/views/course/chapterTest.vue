...
export default {
  ...
  data() {
    return {
      ...
      courseName: "-",
      chapterList: [
        { title: "我的错题测试（100道/次）", id: 1 },
        { title: "高频错题测试（100道/次）", id: 2 },
        { title: "综合题测试（100道/次）", id: 3 },
      ],
      chapterLoading: true,
  ...
  methods: {
    ...
    async queryUserChapterStudy() {
      ...
      this.chapterList.forEach((item) => {
        // 我的错误题
        if (item.id == 1) {
          item.word_time = result.error_testbeforekeyword_time;
          item.test_time = result.error_testbeforeword_time;

          if (result.error_test_score)
            item.test_score = Math.floor(result.error_test_score * 1000) / 10;

          // 如果没有做过单元测试，写入 默认为1%的进度
          if (!result.error_testbeforekeyword_time) item.progress = 1;
        }

        // 高频错题
        if (item.id == 2) {
          item.word_time = result.gaoping_testbeforekeyword_time;
          item.test_time = result.gaoping_testbeforeword_time;

          if (result.gaoping_testbeforeword_score)
            item.test_score =
              Math.floor(result.gaoping_testbeforeword_score * 1000) / 10;

          // 如果有的‘我的错误题’时间，写入 默认为1%的进度
          if (result.error_testbeforekeyword_time) item.progress = 1;
        }

        // 综合测试
        if (item.id == 3) {
          item.word_time = result.zonghe_testbeforekeyword_time;
          item.test_time = result.zonghe_testbeforeword_time;

          if (result.zonghe_testbeforeword_score)
            item.test_score =
              Math.floor(result.zonghe_testbeforeword_score * 1000) / 10;

          // 如果有的‘高频错题’时间，写入 默认为1%的进度
          if (result.gaoping_testbeforeword_score) item.progress = 1;
        }
