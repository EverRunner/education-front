...
export default {
  ...
  methods: {
    ...
    async handleStudyNext() {
      // 判断视频学习是否已经完成过
      if (this.courseProgressStatus > 3) {
        this.handleTabsGoTo(6);
      } else {
        // 更新进度到=> 关键词中英测试
        const isPass = await this.handleStudyStatus(6);
        if (isPass) this.handleTabsGoTo(6);
      }
    },
    ...
    async handleTabsGoTo(type) {
      ...
      switch (type) {
        ...
        case 1:
          // 重学关键词中英
          if (this.courseProgressStatus <= 2) await this.handleStudyStatus(1);
