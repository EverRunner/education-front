...
export default {
  ...
  methods: {
    ...
    async handleStudyNext() {
      ...
      if (this.courseProgressStatus > 4) {
        this.handleTabsGoTo(3);
      } else {
        // 更新进度到=> 关键词中英测试
        const isPass = await this.handleStudyStatus(3);
        if (isPass) this.handleTabsGoTo(3);
      }
    },
    ...
    async handleSubmitMatching() {
      ...
      // 通过测试时，保存章节id
      if (this.passingRate >= 90) {
        const oldData = sessionStorage.getItem("reviewIdList") || "";
        if (oldData) {
          sessionStorage.setItem(
            "reviewIdList",
            oldData + "," + this.params.chapterId
          );
        } else {
          sessionStorage.setItem("reviewIdList", this.params.chapterId);
        }
      }
    ...
    async handleTabsGoTo(type) {
      ...
      switch (type) {
        case 0:
          // 重学视频
          if (this.courseProgressStatus <= 4) await this.handleStudyStatus(0);
        ...
        case 2:
          // 重学关键词英
          if (this.courseProgressStatus <= 4) await this.handleStudyStatus(2);
          name = "COURSE_STUDY_WORD_ENGLISH";
          break;
